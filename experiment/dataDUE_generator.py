...
class DataDUELoader(object):
    ...
    def generate(
        ...
        for i_batch in range(max_batch):
            ...
            feature_indexs, feature_values, labels = self._data_process_before_generate(reactions=reactions_batch)
            yield {
    ...
    def _data_process_before_generate(
        ...
        # feature aggregation #
        # in FM, feature index should be unique over different types #
        index_by_now = 0
        dids = np.expand_dims(dids, axis=-1) + index_by_now
        index_by_now += (0 if self.no_doc_index else self.D)
        users = np.expand_dims(users, axis=-1) + index_by_now
        index_by_now += self.U
        texts = texts + index_by_now
        if self.no_doc_index:
            feature_indexs = np.concatenate(
                [users, texts],
                axis=-1
            )
        else:
        ...
        feature_values = np.ones(feature_indexs.shape, dtype=np.float32)
        labels = emots
        return feature_indexs, feature_values, labels
