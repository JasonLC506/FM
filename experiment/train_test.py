...
def train(
    ...
    model = FM(
        feature_shape=(0 if no_doc_index else data_train.D) + data_train.U + data_train.V + 1,
...
def test(
    ...
    model_spec = json_reader(config_file)
    model = FM(
        feature_shape=(0 if no_doc_index else data.D) + data.U + data.V + 1,
        feature_dim=(0 if no_doc_index else 1) + 1 + max_doc_length,
        label_dim=data.E,
        model_spec=model_spec,
        model_name=model_name
...
class ArgParse(object):
    def __init__(self):
        ...
        parser.add_argument("-cf", "--config_file", default="../models/fm_config.json")
        parser.add_argument("-rp", "--restore_path", default="../ckpt/large_lr_emb16_period_foxnews_nolike/epoch_000")
        parser.add_argument("-mn", "--model_name", default="large_lr_emb16")
        parser.add_argument("-di", "--doc_index", default=True, action="store_true")
